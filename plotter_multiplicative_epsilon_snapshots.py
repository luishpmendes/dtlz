    ...
    for n in ns:
        for m in ms:
            for solver in solvers:
                for seed in seeds:
                    filename = os.path.join(dirname, "multiplicative_epsilon_snapshots/dtlz" + str(dtlz) + "_" + str(n) + "_" + str(m) + "_" + solver + "_" + str(seed) + ".txt")
    ...
    for n in ns:
        for m in ms:
            for i in range(len(solvers)):
                for seed in seeds:
                    filename = os.path.join(dirname, "multiplicative_epsilon_snapshots/dtlz" + str(dtlz) + "_" + str(n) + "_" + str(m) + "_" + solvers[i] + "_" + str(seed) + ".txt")
                    if os.path.exists(filename):
                        with open(filename) as csv_file:
                            data = csv.reader(csv_file, delimiter = ",")
                            j = 0
...
filename = os.path.join(dirname, "multiplicative_epsilon_snapshots/multiplicative_epsilon_mean_snapshots.eps")
