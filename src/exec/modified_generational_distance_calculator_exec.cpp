...
double modified_distance(const std::vector<double> & reference_point,
    ...
    for(unsigned i = 0; i < point.size() && i < reference_point.size(); i++) {
        delta = 0;
...
double modified_inverted_generational_distance(
    ...
    for(unsigned i = 0; i < reference_front.size(); i++) {
        min_distance = modified_distance(reference_front[i],
                                         front.front());

        for(unsigned j = 1; j < front.size(); j++) {
            distance = modified_distance(reference_front[i], front[j]);
...
double normalized_modified_inverted_generational_distance(
        ...
        const std::vector<std::vector<double>> & reference_front,
        const std::vector<std::vector<double>> & front) {
    double igd_plus = modified_inverted_generational_distance(reference_front,
                                                              front);
