...
Solver::Solver(unsigned dtlz) : dtlz(dtlz) {
...
bool Solver::update_best_individuals(
            ...
            const unsigned & m) {
    bool result = false;
    std::vector<NSBRKGA::Sense> senses(m, NSBRKGA::Sense::MINIMIZE);
...
bool Solver::update_best_individuals(
    ...
    return this->update_best_individuals(this->best_individuals,
                                         new_individuals,
                                         this->m,
...
std::ostream & operator <<(std::ostream & os, const Solver & solver) {
    os << "DTLZ: " << solver.dtlz << std::endl
       << "Number of variables: " << solver.n << std::endl
       << "Number of objectives: " << solver.m << std::endl
